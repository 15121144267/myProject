...
public class ClassifySearchListAdapter extends BaseQuickAdapter<ClassifySearchListResponse.DataBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, ClassifySearchListResponse.DataBean item) {
        if (item == null) return;
        helper.setText(R.id.adapter_goods_name, item.resultModel.name);
        helper.setText(R.id.adapter_goods_price, "￥"+ValueUtil.formatAmount( item.resultModel.finalPrice));
        helper.setText(R.id.adapter_goods_count, "销量"+ item.resultModel.saleCount);
        Glide.with(mContext).load( item.resultModel.picture).error(R.mipmap.freemud_logo).into((ImageView) helper.getView(R.id.adapter_person_icon));
    }
