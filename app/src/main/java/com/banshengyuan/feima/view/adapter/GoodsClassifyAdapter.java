...
public class GoodsClassifyAdapter extends BaseQuickAdapter<AllFairListResponse.ListBean, BaseViewHolder> {
    private final Context mContext;
    private final GoodsClassifyControl.GoodsClassifyView mView;

    public GoodsClassifyAdapter(List<AllFairListResponse.ListBean> notices, Context context, GoodsClassifyControl.GoodsClassifyView view) {
    ...
    @Override
    protected void convert(BaseViewHolder helper, AllFairListResponse.ListBean item) {
        if (item == null) return;
        ...
        GoodsClassifyItemAdapter itemAdapter = new GoodsClassifyItemAdapter(item.list1);
